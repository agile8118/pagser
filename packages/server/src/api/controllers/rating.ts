...
const ratePage = async (req: Request, res: Response) => {
  ...
  if (!["like", "dislike"].includes(rate)) {
    throw { status: 400, message: "Invalid rate value" };
  }
...
const rateComment = async (req: Request, res: Response) => {
  const userId = req.user.id;
...
const fetchLikedPages = async (req: Request, res: Response) => {
  const userId = req.user.id;
...
const removeLikedPages = async (req: Request, res: Response) => {
  const userId = req.user.id;
  const { ids } = req.body;
