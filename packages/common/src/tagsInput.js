...
var SEPARATOR = ",";
var COPY_PROPS =
  "placeholder pattern spellcheck autocomplete autocapitalize autofocus accessKey accept lang minLength maxLength required".split(
    " "
  );
function tagsInput(input) {
  ...
  function addTag(text) {
    ...
    if (!input.getAttribute("duplicates")) {
      var _ret = (function () {
        var existingTag = $('[data-tag="' + tag + '"]');
        if (existingTag) {
          existingTag.classList.add("dupe");
          setTimeout(function () {
            return existingTag.classList.remove("dupe");
          }, 100);
          return { v: false };
        }
      })();
  ...
  function setInputWidth() {
    var last = $(".tag", true).pop(),
      w = base.offsetWidth;
    if (!w) return;
    base.input.style.width =
      Math.max(
        w - (last ? last.offsetLeft + last.offsetWidth : 10) - 10,
        w / 4
      ) + "px";
  }
